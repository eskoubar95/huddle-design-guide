 ...
 * Test endpoint for createOrderFromAuction() service method
 * 
 * Body: {
 *   auctionId: string,
 *   buyerId: string,
...
export async function POST(request: NextRequest) {
  try {
    ...
    const order = await medusaOrderService.createOrderFromAuction(
      auctionId,
      buyerId,
      {
    ...
    Sentry.captureException(error, {
      tags: { component: "test_api", endpoint: "create-order-auction" },
      extra: { errorMessage },
    });
