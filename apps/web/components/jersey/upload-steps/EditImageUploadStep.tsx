...
interface EditImageUploadStepProps {
  images: ImageItem[];
...
export const EditImageUploadStep = ({
  images,
  fileInputRef,
  onFileSelect,
  onRemoveImage,
  ...
  onDragOver,
  onDragEnd,
}: EditImageUploadStepProps) => {
  return (
    <div className="space-y-6">
      <div>
        <h3 className="text-lg font-bold mb-2">Images (1-5 photos)</h3>
        <p className="text-sm text-muted-foreground mb-4">
          Drag to reorder. Click to remove. First image will be the cover.
        </p>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-4">
          {images.map((image, index) => (
            ...
            >
              ...
              {image.isNew && (
                <BadgeUI variant="secondary" className="absolute bottom-2 left-2">
                  New
                </BadgeUI>
              )}
            </div>
