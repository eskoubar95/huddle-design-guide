...
-- Using NUMERIC(10,2) for compatibility, but values must be whole cents
ALTER TABLE public.transactions
ADD COLUMN IF NOT EXISTS item_amount NUMERIC(10,2) NULL;

COMMENT ON COLUMN public.transactions.item_amount IS 'Item price in cents (minor units, integer). Example: 1050 = €10.50';

-- Ensure only whole cents are stored (no fractional cents)
ALTER TABLE public.transactions
ADD CONSTRAINT check_item_amount_whole_cents 
CHECK (item_amount IS NULL OR item_amount = FLOOR(item_amount));

-- Add shipping_amount column (shipping cost in cents)
ALTER TABLE public.transactions
ADD COLUMN IF NOT EXISTS shipping_amount NUMERIC(10,2) NULL;

COMMENT ON COLUMN public.transactions.shipping_amount IS 'Shipping cost in cents (minor units, integer). NULL until shipping is determined. Example: 500 = €5.00';

ALTER TABLE public.transactions
ADD CONSTRAINT check_shipping_amount_whole_cents 
CHECK (shipping_amount IS NULL OR shipping_amount = FLOOR(shipping_amount));
...
COMMENT ON COLUMN public.transactions.platform_fee_amount IS 'Platform fee in cents (minor units, integer), charged to buyer. Calculated as % of item_amount. Example: 53 = €0.53';

ALTER TABLE public.transactions
ADD CONSTRAINT check_platform_fee_amount_whole_cents 
CHECK (platform_fee_amount IS NULL OR platform_fee_amount = FLOOR(platform_fee_amount));
...
COMMENT ON COLUMN public.transactions.seller_fee_amount IS 'Seller fee in cents (minor units, integer), deducted from seller payout. Refundable. Example: 11 = €0.11';

ALTER TABLE public.transactions
ADD CONSTRAINT check_seller_fee_amount_whole_cents 
CHECK (seller_fee_amount IS NULL OR seller_fee_amount = FLOOR(seller_fee_amount));

-- Add total_amount column (total buyer pays in cents)
ALTER TABLE public.transactions
ADD COLUMN IF NOT EXISTS total_amount NUMERIC(10,2) NULL;

COMMENT ON COLUMN public.transactions.total_amount IS 'Total amount buyer pays in cents (minor units, integer). Equals item_amount + shipping_amount + platform_fee_amount. Example: 1603 = €16.03. NULL for auctions until checkout.';

ALTER TABLE public.transactions
ADD CONSTRAINT check_total_amount_whole_cents 
CHECK (total_amount IS NULL OR total_amount = FLOOR(total_amount));
...
COMMENT ON COLUMN public.transactions.seller_payout_amount IS 'Net payout to seller in cents (minor units, integer). Equals item_amount - seller_fee_amount. Example: 1039 = €10.39';

ALTER TABLE public.transactions
ADD CONSTRAINT check_seller_payout_amount_whole_cents 
CHECK (seller_payout_amount IS NULL OR seller_payout_amount = FLOOR(seller_payout_amount));
